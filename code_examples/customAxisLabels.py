...
class CustomTextItem(gl.GLGraphicsItem.GLGraphicsItem):
    ...
    def paint(self):
        ...
        self.GLViewWidget.renderText(self.X, self.Y, self.Z, self.text, font=QFont('Arial', pointSize=12, weight=150))
...
class Custom3DAxis(gl.GLAxisItem):
    ...
    def add_labels(self):
        ...
        x,y,z = self.size()
        #X label
        self.xLabel = CustomTextItem(X=x/2, Y=-y/20 - 1, Z=-z/20, text="X")
        self.xLabel.setGLViewWidget(self.parent)
        self.parent.addItem(self.xLabel)

        #Y label
        self.yLabel = CustomTextItem(X=-x/20 - 1, Y=y/2, Z=-z/20, text="Y")
        self.yLabel.setGLViewWidget(self.parent)
        self.parent.addItem(self.yLabel)

        #Z label
        self.zLabel = CustomTextItem(X=-x/20 - 1, Y=-y/20 - 1, Z=z/2, text="Z")
...
axis = Custom3DAxis(fig1, color=(1.,1.,1.,.8))
